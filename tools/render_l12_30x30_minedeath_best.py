"""Render the 30x30 mine=death GRPO best on L12 dispextreme: keep retrying
seeds and pick the longest successful rollout so the GIF actually shows
the policy threading walls + mines, not a near-spawn exit."""
...
import tools.run_finetune_minedeath_30x30 as ft30

RUN_ROOT = REPO_ROOT / "minefield_rl/logs/finetune_minedeath_30_run_20260426_162653"
GRPO_BEST = RUN_ROOT / "grpo/checkpoints/ppo_grpo_best.pt"

OUT_PREFIX = REPO_ROOT / "minefield_rl/logs/l12_30x30_minedeath_render"
OUT_GIF = REPO_ROOT / "assets/l12_grpo_minedeath_30x30_every_step.gif"
OUT_MP4 = REPO_ROOT / "assets/l12_grpo_minedeath_30x30_every_step.mp4"

NUM_ATTEMPTS = 32
MIN_STEPS = 18           # demand a non-trivial rollout under mine=death
HOLD_FRAMES = 3
EARLY_STOP_STEPS = 40    # stop once we hit a really nice one

HEADER = "30x30 dispersed extreme | GRPO best | L12 (0.40w / 0.30m) | max_health = 1 | every step, 1 fps"
...
def main() -> None:
    ...
    profile = ft30.PROFILES["L12_dispextreme"]
    cfg = deepcopy(base_cfg)
    cfg = deep_update(cfg, {"env": ft30._stage_env_overrides(profile)})
    ...
    footer = f"map_seed {selected['seed']}  |  {selected['steps']} steps  |  success (mine = instant death)"
