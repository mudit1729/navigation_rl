"""30x30 dispersed extension to the progressive curriculum.

Mirrors tools/run_progressive20_dispersed.py but at 30x30. The model's 9x9
egocentric view is grid-size-agnostic, so weights from the 20x20 dispersed
GRPO best transfer cleanly. Run sequence:

  L10 Dispersed Walls    — 0.40 walls, 0.20 mines, dispersed, 30x30
  L11 Dispersed Mines    — 0.30 walls, 0.30 mines, dispersed, 30x30
  L12 Dispersed Extreme  — 0.40 walls, 0.30 mines, dispersed, 30x30
...
(at 30x30) plus the new dispersed profiles.

Warm-starts from the post-fix L13 GRPO 20x20 best (the one produced by
tools/redo_grpo_stages.py with the corrected MCTS off-policy ratio),
not the older pre-fix checkpoint.
"""
...
# Post-fix GRPO best from the redo_grpo rerun (corrected MCTS off-policy ratio).
PRIOR_RUN_ROOT = REPO_ROOT / "minefield_rl" / "logs" / "grpo_rerun_20260426_141043"
PRIOR_GRPO_BEST = PRIOR_RUN_ROOT / "L13_dispersed_20x20" / "grpo" / "checkpoints" / "ppo_grpo_best.pt"

GRID_SIZE = 30

# max_steps roughly tracks (shortest path × detour budget). At 20x20 we used
# 480-1000; scaling to 30x30 by ~1.7x gives the policy room to recover from
# bad alleys without inflating training compute.
PROFILES = {
    "L1_open":            {"name": "L1_open",            "wall_density": 0.10, "mine_density": 0.05, "max_steps": 720,  "generation_max_attempts": 64,   "revisit_window": 12, "dispersion": "clustered"},
    "L4_minedmaze":       {"name": "L4_minedmaze",       "wall_density": 0.30, "mine_density": 0.20, "max_steps": 1200, "generation_max_attempts": 256,  "revisit_window": 12, "dispersion": "clustered"},
    "L10_dispwalls":      {"name": "L10_dispwalls",      "wall_density": 0.40, "mine_density": 0.20, "max_steps": 1200, "generation_max_attempts": 1024, "revisit_window": 12, "dispersion": "dispersed"},
    "L11_dispmines":      {"name": "L11_dispmines",      "wall_density": 0.30, "mine_density": 0.30, "max_steps": 1200, "generation_max_attempts": 1024, "revisit_window": 12, "dispersion": "dispersed"},
    "L12_dispextreme":    {"name": "L12_dispextreme",    "wall_density": 0.40, "mine_density": 0.30, "max_steps": 1500, "generation_max_attempts": 4096, "revisit_window": 12, "dispersion": "dispersed"},
    "L13_dispopen":       {"name": "L13_dispopen",       "wall_density": 0.20, "mine_density": 0.20, "max_steps": 900,  "generation_max_attempts": 512,  "revisit_window": 12, "dispersion": "dispersed"},
}
...
def _stage_env_overrides(profile: dict, profiles_list: list[dict] | None = None) -> dict:
    return {
        ...
        "scenario_name": f"progressive30_{profile['name']}",
...
def _build_l13_config(base_config: dict, demos: int, ppo_steps: int, grpo_updates: int) -> dict:
    ...
    return deep_update(cfg, {
        "env": {
            ...
            "view_radius": 4,
            "max_steps": 1500,
            "generation_max_attempts": 4096,
            "revisit_window": 12,
            "dispersion": "dispersed",
            "scenario_name": "progressive30_L13_dispmixed",
...
def main() -> None:
    if not PRIOR_GRPO_BEST.exists():
        raise FileNotFoundError(f"prior 20x20 GRPO checkpoint missing: {PRIOR_GRPO_BEST}")
    ...
    root = ensure_dir(REPO_ROOT / "minefield_rl" / "logs" / f"progressive30_disp_run_{timestamp}")
    summary_path = root / "summary.json"
    results: dict = {
        "prior_run": str(PRIOR_RUN_ROOT),
        "prior_checkpoint": str(PRIOR_GRPO_BEST),
        "grid_size": GRID_SIZE,
    }
