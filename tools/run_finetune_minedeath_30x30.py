"""Fine-tune the 30x30 dispersed L13 PPO best with mine = instant death.

Mirrors tools/run_finetune_minedeath.py but at 30x30. Sets max_health=1 so
any mine hit terminates the episode. The expert planner refuses to step on
mines when health <= 1, so demos are strictly mine-free paths.

Pipeline:
  IL fine-tune  (warm from 30x30 L13 PPO best, dispersed mix, max_health=1)
...
PRIOR_RUN_ROOT = REPO_ROOT / "minefield_rl" / "logs" / "progressive30_disp_run_20260426_141827"
...
def _stage_env_overrides(profile: dict, profiles_list: list[dict] | None = None) -> dict:
    return {
        ...
        "scenario_name": f"finetune_minedeath_30_{profile['name']}",
...
def _build_mix_config(base_config: dict) -> dict:
    ...
    return deep_update(cfg, {
        "env": {
            ...
            "scenario_name": "finetune_minedeath_30_mix",
...
def main() -> None:
    if not PRIOR_PPO_BEST.exists():
        raise FileNotFoundError(f"prior 30x30 L13 PPO checkpoint missing: {PRIOR_PPO_BEST}")
    ...
    root = ensure_dir(REPO_ROOT / "minefield_rl" / "logs" / f"finetune_minedeath_30_run_{timestamp}")
