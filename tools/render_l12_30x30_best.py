"""Render the 30x30 L12 dispextreme PPO best: keeps retrying seeds and picks
the longest successful rollout, so the GIF actually shows the policy
navigating dense walls + mines on the larger grid."""
...
import tools.run_progressive30_dispersed as p30

RUN_ROOT = REPO_ROOT / "minefield_rl/logs/progressive30_disp_run_20260426_141827"
PPO_BEST = RUN_ROOT / "L12_dispextreme/ppo/checkpoints/ppo_best.pt"

OUT_PREFIX = REPO_ROOT / "minefield_rl/logs/l12_30x30_render"
OUT_GIF = REPO_ROOT / "assets/l12_ppo_30x30_every_step.gif"
OUT_MP4 = REPO_ROOT / "assets/l12_ppo_30x30_every_step.mp4"

NUM_ATTEMPTS = 24
MIN_STEPS = 25       # demand a non-trivial rollout (30x30 paths can be 30-100+ steps)
HOLD_FRAMES = 3

HEADER = "30x30 dispersed extreme | PPO best | L12 (0.40w / 0.30m) | every step, 1 fps"
...
def main() -> None:
    if not PPO_BEST.exists():
        sys.exit(f"missing PPO ckpt: {PPO_BEST}")
    ...
    profile = p30.PROFILES["L12_dispextreme"]
    cfg = deepcopy(base_cfg)
    cfg = deep_update(cfg, {"env": p30._stage_env_overrides(profile)})
    ...
    renderer = PygameRenderer(
        config=cfg, agent="ppo", checkpoint_path=str(PPO_BEST), device="cpu",
    )
    renderer.speed_index = len(renderer.speed_modes) - 1
    renderer.show_full_map = False

    best: dict | None = None
    fallback: dict | None = None  # longest success even if below MIN_STEPS
    for attempt in range(1, NUM_ATTEMPTS + 1):
        ...
        print(f"attempt {attempt:2d}: outcome={outcome:8s} steps={steps:3d} map_seed={seed}")
        if outcome == "success":
            cand = {"dir": ep_dir, "steps": steps, "frames": idx, "seed": seed}
            if steps >= MIN_STEPS:
                if best is None or steps > best["steps"]:
                    best = cand
                    if steps >= 50:  # plenty good — stop early
                        break
            else:
                if fallback is None or steps > fallback["steps"]:
                    fallback = cand
    ...
    if selected is None:
        sys.exit("no successful rollouts produced")
    print(f"\nselected: steps={selected['steps']} frames={selected['frames']} seed={selected['seed']}")
    ...
    pngs = sorted(selected["dir"].glob("f_*.png"))
    footer = f"map_seed {selected['seed']}  |  {selected['steps']} steps  |  success"
